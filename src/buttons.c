...
#include "freertos/task.h" // Aquí debe de ser #include "freertos/task.h" y no #include "freertos/tasks.h"
...
static bool debounce_update(uint8_t gpio, Debounce_t *db)
{
    ...
    if (raw == db->last_raw)
    {
        ...
        {
            db->count++; //Acá debe de sumar no restar
        }
    }
    else
    {
    ...
    if (db->count >= DEBOUNCE_COUNT)
    {
        if (raw != db->stable_state)
        {
            ...
            */
            ...
            //Falta un caso para poder cambiar el estado de pressed_event
            else if (db->stable_state == 0) 
            {
                pressed_event = true; 
            }
        }
...
void button_task(void *pvParameters)
{
    ...
    while (1)
    {
        ...
        vTaskDelay(pdMS_TO_TICKS(BUTTON_POLL_MS)); // Aquí debe de ser BUTTON_POLL_MS y no BUTTONS_POLL_MS
    }
}